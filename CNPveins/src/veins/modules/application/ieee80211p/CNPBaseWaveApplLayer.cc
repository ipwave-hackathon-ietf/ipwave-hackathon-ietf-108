...
////        end BAM
        ...
        if (strcmp(findHost()->getFullName(), "node[2]") == 0){
            scheduleAt(simTime() + 60, stopvehicle);
        }
...
void CNPBaseWaveApplLayer::handleLowerMsg(cMessage* msg) {
    ...
    if (CooperativeContMsg* bsm = dynamic_cast<CooperativeContMsg*>(wsm)) {
        std::cout <<"CCM msg received by : " <<findHost()->getFullName()<<std::endl;
    ...
    else if (EmergentContextMsg* pkt = dynamic_cast<EmergentContextMsg*>(wsm)){
        std::cout <<"ECM msg received by : " <<findHost()->getFullName()<<std::endl;
        if (strcmp(findHost()->getName(), "node") == 0){
...
void CNPBaseWaveApplLayer::finish() {
    if (strcmp(findHost()->getName(), "node") == 0){
        if (strcmp(mobility->getExternalId().c_str(), ObstacleInfo.vehicleId.c_str()) == 0){
            ...
            ofstream res;
            res.open("/home/iotlab/Documents/Final-Exam/CNPveins/examples/veins/results/collprobres.csv");
            res << "Cluster_ID: ,"
                       <<"Number_Members: ,"
                       <<"Em_CP: ,"
                       <<"Time_Stamp: ,"
            ...
            ofstream acc;
            acc.open("/home/iotlab/Documents/Final-Exam/CNPveins/examples/veins/results/accidentres.csv");
